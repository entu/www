...
Parameetrid on kaardi väljad — nimega väärtused, mis objekti kirjeldavad. Just need teevad iga objektitüübi ainulaadseks: `person` objektil on `name` ja `email`; `invoice` objektil on `amount` ja `due_date`. Väljad, nende tüübid ja reeglid valid sina — ja saad neid muuta alati, kui vajadused muutuvad.

Parameetrid on määratletud objektitüübil (objektitüübi `property` tüüpi alam-objektidena). Vaata, kuidas neid kasutajaliideses seadistada [Objektitüübid](/et/seadistamine/objektituubid/#parameetrite-definitsioonide-lisamine) alt.
...
**Arvutatud** — Sea parameetri definitsioonil `formula`, et arvutada selle väärtus automaatselt iga salvestamisega. Vaata [Valemid](/et/api/valemid/).
...
Eri keelte väärtused salvestatakse eraldi parameetriobjektidena, millest igaüks kannab `language` koodi. Vaata API formaadi kohta [API → Parameetrid](/et/api/parameetrid/).
...
Täieliku formaadi kohta vaata [Parameetrite vaikeväärtused](/et/seadistamine/objektituubid/#parameetrite-vaikevaartused).
...
Täieliku süntaksi kohta vaata [Valemid](/et/api/valemid/).
...
| `photo` | file | Esimest väärtust kasutatakse objekti pisipildina loendites ja objekti lehe päises. Pisipildid genereeritakse nõudmisel objekti pisipildi endpointi kaudu — vaata [Failid → Pisipildid](/et/api/failid/#pisipildid). |
